...
### FILE SOURCE ###
seasonality <- read_excel("W:\\Business Intelligence\\LIFESTYLE\\10. Forecast & Reorder\\SEASONALITY\\Lifestyle Seasonality 2111.xlsx", skip = 3)
...
### SEASONALITY ###
seasonality <- seasonality[1:52, c(3,22)]
names(seasonality)[names(seasonality) == "WEEK #...3"] <- "Week"
names(seasonality)[names(seasonality) == "SEASONAL CURVE 100%"] <- "Weight"

### MAIN DATASET FOR FORECAST ###
# STEP 1 TEMPLATE 2 COMPLETE YEARS WITH SALES / INV / PFP DATA
w22 <- tibble('Week' = 1:52) %>% mutate('Year' = 2022)
w23 <- tibble('Week' = 1:52) %>% mutate('Year' = 2023)
main <- rbind(w22, w23) %>% merge(seasonality, by='Week')

# ### C'EST TROP LENT // TROUVER UNE AUTRE METHODE ###
tmp <- list()
for(i in 1:length(unique(inv2$SMC))) {
  tmp[[i]] <- full_join(main, filter(inv2, SMC == inv2$SMC[i]), by = c("Week" , "Year")) %>%
    fill(SMC, Department, Line, Season, .direction = "updown")
}

base <- do.call(rbind, tmp)
base <- left_join(base, sales2, by = c("Week" , "Year", "SMC", "Department", "Line", "Season"))
base <- left_join(base, pfp2, by = c("Week" , "Year", "SMC", "Department", "Line", "Season"))
base <- base %>% arrange(SMC, Year, Week, Department, Line, Season) %>% mutate(Time = paste(Week,Year,sep = "-"))
base[is.na(base)] <- 0
